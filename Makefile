...
run-local: package check-env
	docker run -d -v ~/.aws/:/root/.aws/:ro -p 5000:5000 -e AWS_REGION=$(AWS_REGION) -e NOTIFICATION_TOPIC=arn:aws:sns:$(AWS_REGION):$(ACCOUNT_ID):scorekeep-notifications scorekeep-api
